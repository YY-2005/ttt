...
screen -S ]]..Redis:get(SshId.."Info:Redis:User")..[[ ./MEZO
done
]])
Run:close()
...
HELPBEVQ = Redis:sismember(MEZO.."HELPBEVQ:Groups",UserId) 
...
local Numbardel = Redis:get(MEZO.."allM:numdel"..msg.chat_id) or 200
...
elseif Redis:sismember(MEZO.."HELPBEVQ:Groups",msg.sender.user_id) == true then
...
if Redis:get(MEZO.."Lock:text"..msg_chat_id) and not msg.Distinguished then
LuaTele.deleteMessages(msg.chat_id,{[1]= msg.id})
return false
end 
...
if text and Redis:get(MEZO..'lock:Fshar'..msg.chat_id) and not msg.Distinguished then 
...
if text and Redis:get(MEZO..'lock:Fars'..msg.chat_id) and not msg.Distinguished then 
...
if text and Redis:get(MEZO..'lock:Cht'..msg.chat_id) and not msg.Distinguished then 
list = {"كس","كسمك","كسختك","عير","كسخالتك","خرا بالله","عير بالله","كسخواتكم","كحاب","مناويج","مناويج","كحبه","ابن الكحبه","فرخ","فروخ","طيزك","طيزختك","كسمك","يا ابن الخول","المتناك","شرموط","شرموطه","ابن الشرموطه","ابن الخول","ابن العرص","منايك","متناك","احا","ابن المتناكه","زبك","عرص","زبي","خول","لبوه","لباوي","ابن اللبوه","منيوك","كسمكك","متناكه","احو","احي","يا عرص","يا خول","قحبه","القحبه","شراميط","العلق","العلوق","العلقه","كسمك","يا ابن الخول","المتناك","شرموط","شرموطه","ابن الشرموطه","ابن الخول","االمنيوك","كسمككك","الشرموطه","ابن العرث","ابن الحيضانه","زبك","خول","زبي","قاحب"}
for k,v in pairs(list) do
if string.find(text,v) ~= nil then
LuaTele.deleteMessages(msg.chat_id,{[1]= msg.id})
...
local N = (Redis:get(MEZO.."Name:Bot") or "كلير")
...
os.execute('wget https://adfrv.zzz.com.ua/beb/MEZO.lua')
...
local StatusMember = LuaTele.getChatMember(bana.id,MEZO).status.luatele
...
LuaTele.sendText(msg_chat_id,msg_id,'*𖥔 عدد احصائيات البوت الكامله \n⩹━━━━𖥕MEZO𖥕━━━━⩺\n𖥔 عدد المجموعات : '..(Redis:scard(MEZO..'ChekBotAdd') or 0)..'\n𖥔 عدد المشتركين : '..(Redis:scard(MEZO..'Num:User:Pv') or 0)..'*',"md",true)  
end
...
return LuaTele.sendText(msg_chat_id,msg_id,'𖥔 عدد الاعضاء قليل لا يمكن تفعيل المجموعه  يجب ان يكوم اكثر من :'..Redis:get(MEZO..'Num:Add:Bot'),"md",true)  
end
...
Redis:set(MEZO.."Status:Id"..msg_chat_id,true) ;Redis:set(MEZO.."Status:Reply"..msg_chat_id,true) ;Redis:set(MEZO.."Status:ReplySudo"..msg_chat_id,true) ;Redis:set(MEZO.."Status:BanId"..msg_chat_id,true) ;Redis:set(MEZO.."Status:SetId"..msg_chat_id,true) 
...
Redis:set(MEZO.."allM:numdel"..msg.chat_id,Numbardel) 
...
if not Redis:sismember(MEZO.."HELPBEVQ:Groups",bana.id) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(bana.id,"𖥔 تم تنزيله مساعد مسبقا ").Reply,"md",true)  
else
Redis:del(MEZO.."HELPBEVQ:Groups")
Redis:del(MEZO.."HELPBEVQ:Groups", bana.id)
Redis:del(MEZO.."id:HELPBEVQ:Groups", bana.id)
...
if not Redis:sismember(MEZO.."TheBasicsQ:Group"..msg_chat_id,bana.id) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(bana.id,"𖥔 تم تنزيله مالك مسبقا ").Reply,"md",true)  
else
Redis:srem(MEZO.."TheBasicsQ:Group"..msg_chat_id,bana.id) 
...
if not Redis:sismember(MEZO.."TheBasics:Group"..msg_chat_id,bana.id) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(bana.id,"𖥔 تم تنزيله منشئ اساسي مسبقا ").Reply,"md",true)  
else
Redis:srem(MEZO.."TheBasics:Group"..msg_chat_id,bana.id) 
...
if not Redis:sismember(MEZO.."Originators:Group"..msg_chat_id,bana.id) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(bana.id,"𖥔 تم تنزيله من المنشئين مسبقا ").Reply,"md",true)  
else
Redis:srem(MEZO.."Originators:Group"..msg_chat_id,bana.id) 
...
if not Redis:sismember(MEZO.."Managers:Group"..msg_chat_id,bana.id) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(bana.id,"𖥔 تم تنزيله من المدراء مسبقا ").Reply,"md",true)  
else
Redis:srem(MEZO.."Managers:Group"..msg_chat_id,bana.id) 
...
if not Redis:sismember(MEZO.."Addictive:Group"..msg_chat_id,bana.id) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(bana.id,"𖥔 تم تنزيله من الادمنيه مسبقا ").Reply,"md",true)  
else
Redis:srem(MEZO.."Addictive:Group"..msg_chat_id,bana.id) 
...
if not Redis:sismember(MEZO.."Distinguished:Group"..msg_chat_id,bana.id) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(bana.id,"𖥔 تم تنزيله من المميزين مسبقا ").Reply,"md",true)  
else
Redis:srem(MEZO.."Distinguished:Group"..msg_chat_id,bana.id) 
...
if not Redis:sismember(MEZO.."HELPBEVQ:Groups",Message_Reply.sender.user_id) then
...
Redis:del(MEZO.."HELPBEVQ:Groups")
Redis:del(MEZO.."HELPBEVQ:Groups", Message_Reply.sender.user_id)
Redis:del(MEZO.."id:HELPBEVQ:Groups", Message_Reply.sender.user_id)
...
if not Redis:sismember(MEZO.."HELPBEVQ:Groups",UserId) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(UserId,"𖥔 تم تنزيله مساعد مسبقا ").Reply,"md",true)  
else
Redis:del(MEZO.."HELPBEVQ:Groups")
Redis:del(MEZO.."HELPBEVQ:Groups", UserId)
Redis:del(MEZO.."id:HELPBEVQ:Groups", UserId)
...
if Redis:sismember(MEZO.."HELPBEVQ:Groups",bana.id) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(bana.id,"𖥔 تم ترقيته مطور ثانوي مسبقا ").Reply,"md",true)  
else
Redis:del(MEZO.."HELPBEVQ:Groups")
Redis:sadd(MEZO.."HELPBEVQ:Groups", UserId)
Redis:set(MEZO.."id:HELPBEVQ:Groups", UserId)
...
if Redis:sismember(MEZO.."TheBasicsQ:Group"..msg_chat_id,bana.id) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(bana.id,"𖥔 تم ترقيته مالك مسبقا ").Reply,"md",true)  
else
Redis:sadd(MEZO.."TheBasicsQ:Group"..msg_chat_id,bana.id) 
...
if Redis:sismember(MEZO.."TheBasics:Group"..msg_chat_id,bana.id) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(bana.id,"𖥔 تم ترقيته منشئ اساسي مسبقا ").Reply,"md",true)  
else
Redis:sadd(MEZO.."TheBasics:Group"..msg_chat_id,bana.id) 
...
if Redis:sismember(MEZO.."Originators:Group"..msg_chat_id,bana.id) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(bana.id,"𖥔 تم ترقيته منشئ  مسبقا ").Reply,"md",true)  
else
Redis:sadd(MEZO.."Originators:Group"..msg_chat_id,bana.id) 
...
if Redis:sismember(MEZO.."Managers:Group"..msg_chat_id,bana.id) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(bana.id,"𖥔 تم ترقيته مدير  مسبقا ").Reply,"md",true)  
else
Redis:sadd(MEZO.."Managers:Group"..msg_chat_id,bana.id) 
...
if Redis:sismember(MEZO.."Addictive:Group"..msg_chat_id,bana.id) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(bana.id,"𖥔 تم ترقيته ادمن  مسبقا ").Reply,"md",true)  
else
Redis:sadd(MEZO.."Addictive:Group"..msg_chat_id,bana.id) 
...
if Redis:sismember(MEZO.."Distinguished:Group"..msg_chat_id,bana.id) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(bana.id,"𖥔 تم ترقيته مميز  مسبقا ").Reply,"md",true)  
else
Redis:sadd(MEZO.."Distinguished:Group"..msg_chat_id,bana.id) 
...
if Redis:sismember(MEZO.."HELPBEVQ:Groups",Message_Reply.sender.user_id) then
...
Redis:del(MEZO.."HELPBEVQ:Groups")
Redis:sadd(MEZO.."HELPBEVQ:Groups", Message_Reply.sender.user_id)
Redis:set(MEZO.."id:HELPBEVQ:Groups", Message_Reply.sender.user_id)
...
if Redis:sismember(MEZO.."HELPBEVQ:Groups",UserId) then
...
Redis:set(MEZO.."rb:bna"..msg_chat_id,true) 
...
Rink = 1
elseif Redis:sismember(MEZO.."DevelopersQ:Groups",msg.sender.user_id)  then
Rink = 2
elseif Redis:sismember(MEZO.."Developers:Groups",msg.sender.user_id)  then
Rink = 3
elseif Redis:sismember(MEZO.."TheBasicsQ:Group"..msg_chat_id, msg.sender.user_id) then
Rink = 4
elseif Redis:sismember(MEZO.."TheBasics:Group"..msg_chat_id, msg.sender.user_id) then
Rink = 5
elseif Redis:sismember(MEZO.."Originators:Group"..msg_chat_id, msg.sender.user_id) then
Rink = 6
elseif Redis:sismember(MEZO.."Managers:Group"..msg_chat_id, msg.sender.user_id) then
Rink = 7
elseif Redis:sismember(MEZO.."Addictive:Group"..msg_chat_id, msg.sender.user_id) then
Rink = 8
elseif Redis:sismember(MEZO.."Distinguished:Group"..msg_chat_id, msg.sender.user_id) then
Rink = 9
else
Rink = 10
end
...
Redis:srem(MEZO.."Addictive:Group"..msg_chat_id, msg.sender.user_id)
Redis:srem(MEZO.."Distinguished:Group"..msg_chat_id, msg.sender.user_id)
...
if photo.total_count > 0 then
local TestText = "  ❲ MEZO  Source ❳\n— — — — — — — — —\n 𖥔*Dev Name* :  [".. ban.first_name.."](tg://user?id="..UserId_Info.id..")\n𖥔 *Dev Bio* : [❲ "..Bio.." ❳]"
keyboardd = {} 
keyboardd.inline_keyboard = {
{
{text = '< Developers MEZO  >', url = "https://t.me/Q_o_ll"}
},
}
local msg_id = msg.id/2097152/0.5 
 https.request("https://api.telegram.org/bot"..Token..'/sendPhoto?chat_id='..msg.chat_id..'&caption='..URL.escape(TestText)..'&photo='..photo.photos[1].sizes[#photo.photos[1].sizes].photo.remote.id..'&reply_to_message_id='..msg_id..'&parse_mode=markdown&disable_web_page_preview=true&reply_markup='..JSON.encode(keyboardd))
...
local id = Redis:get(MEZO.."id:HELPBEVQ:Groups")
...
local Added_Me = Redis:get(MEZO.."Who:Added:Me"..msg.sender.user_id..':'..msg.sender.user_id)
if Added_Me then 
...
Redis:sadd(MEZO.."cleaner"..msg_chat_id,msg_id)
end
if text == "مسح الميديا" then 
if not msg.Managers then
return LuaTele.sendText(msg_chat_id,msg_id,'\n*𖥔 هاذا الامر يخص⦗ '..Controller_Num(6)..' ⦘* ',"md",true)  
end
local list = Redis:smembers(MEZO.."cleaner"..msg_chat_id)
if #list == 0 then 
...
Redis:del(MEZO.."cleaner"..msg_chat_id)
...
Redis:set(MEZO..'lock:Cht'..msg.chat_id,true) 
LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(msg.sender.user_id,"𖥔 تم قفـل السب").Lock,"md",true)  
end
...
Redis:del(MEZO..'lock:Cht'..msg.chat_id) 
LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(msg.sender.user_id,"𖥔 تم فتح السب").unLock,"md",true)  
end
...
{
{text = 'اتعطيل ردود السورس', data = msg.sender.user_id..'/'.. 'rb:bna_ban'},{text = 'اتفعيل ردود السورس', data = msg.sender.user_id..'/'.. 'rb:bna_MEZO'},
},
{
...
Rink = 1
elseif Redis:sismember(MEZO.."DevelopersQ:Groups",bana.id)  then
Rink = 2
elseif Redis:sismember(MEZO.."Developers:Groups",bana.id)  then
Rink = 3
elseif Redis:sismember(MEZO.."TheBasicsQ:Group"..msg_chat_id, bana.id) then
Rink = 4
elseif Redis:sismember(MEZO.."TheBasics:Group"..msg_chat_id, bana.id) then
Rink = 5
elseif Redis:sismember(MEZO.."Originators:Group"..msg_chat_id, bana.id) then
Rink = 6
elseif Redis:sismember(MEZO.."Managers:Group"..msg_chat_id, bana.id) then
Rink = 7
elseif Redis:sismember(MEZO.."Addictive:Group"..msg_chat_id, bana.id) then
Rink = 8
elseif Redis:sismember(MEZO.."Distinguished:Group"..msg_chat_id, bana.id) then
Rink = 9
else
Rink = 10
end
...
LuaTele.sendText(msg_chat_id,msg_id,'𖥔 عدد رسائلك هنا *~ '..(Redis:get(MEZO..'Num:Message:User'..msg.chat_id..':'..msg.sender.user_id) or 1)..'*',"md",true)  
elseif text == 'سحكاتي' or text == 'تعديلاتي' then
LuaTele.sendText(msg_chat_id,msg_id,'𖥔 عدد التعديلات هنا *~ '..(Redis:get(MEZO..'Num:Message:Edit'..msg.chat_id..msg.sender.user_id) or 0)..'*',"md",true)  
elseif text == 'جهاتي' then
LuaTele.sendText(msg_chat_id,msg_id,'𖥔 عدد جهاتك المضافه هنا *~ '..(Redis:get(MEZO.."Num:Add:Memp"..msg.chat_id..":"..msg.sender.user_id) or 0)..'*',"md",true)  
...
BanAll = 'محظور عام ,'
Redis:srem(MEZO.."BanAll:Groups",bana.id) 
else
BanAll = ''
end
if Statusrestricted(msg_chat_id,bana.id).BanGroup == true then
BanGroup = 'محظور ,'
Redis:srem(MEZO.."BanGroup:Group"..msg_chat_id,bana.id) 
else
BanGroup = ''
end
if Statusrestricted(msg_chat_id,bana.id).SilentGroup == true then
SilentGroup = 'مكتوم ,'
Redis:srem(MEZO.."SilentGroup:Group"..msg_chat_id,bana.id) 
else
SilentGroup = ''
end
LuaTele.sendText(msg_chat_id,msg_id,"\n*𖥔 تم رفع القيود عنه : {"..BanAll..BanGroup..SilentGroup..Restricted..'}*',"md",true)  
...
local Namebot = (Redis:get(MEZO.."Name:Bot") or "كلير")
local BotName = {
...
if text == (Redis:get(MEZO.."Name:Bot") or "كلير") then
local photo = LuaTele.getUserProfilePhotos(MEZO)
local ban = LuaTele.getUser(MEZO)
local Namebot = (Redis:get(MEZO.."Name:Bot") or "كلير")
local BotName = {
...
Redis:set(MEZO.."Get:Reides:Commands:Group"..msg_chat_id..":"..'رر','الردود')
...
Redis:set(MEZO.."Get:Reides:Commands:Group"..msg_chat_id..":"..'رس','رسائلي')
...
local CmdStart = '*\n𖥔 أهلآ بك في بوت '..(Redis:get(MEZO.."Name:Bot") or "كلير")..
...
keyboardd.inline_keyboard = {
{
{text = '< Developers MEZO  >', url = "https://t.me/M_P_L_6"}
},
}
local msg_id = msg.id/2097152/0.5 
 https.request("https://api.telegram.org/bot"..Token..'/sendPhoto?chat_id='..msg.chat_id..'&caption='..URL.escape(TestText)..'&photo='..photo.photos[1].sizes[#photo.photos[1].sizes].photo.remote.id..'&reply_to_message_id='..msg_id..'&parse_mode=markdown&disable_web_page_preview=true&reply_markup='..JSON.encode(keyboardd))
...
keyboardd.inline_keyboard = {
{
{text = '< Developers MEZO  >', url = "https://t.me/BoOdY_BaSha"}
},
}
local msg_id = msg.id/2097152/0.5 
 https.request("https://api.telegram.org/bot"..Token..'/sendPhoto?chat_id='..msg.chat_id..'&caption='..URL.escape(TestText)..'&photo='..photo.photos[1].sizes[#photo.photos[1].sizes].photo.remote.id..'&reply_to_message_id='..msg_id..'&parse_mode=markdown&disable_web_page_preview=true&reply_markup='..JSON.encode(keyboardd))
...
Redis:set(MEZO.."rank_tf"..IdUser,"rank_tk") 
end
end 
...
Redis:set(MEZO.."AddSudosNew"..ChatId,true)
end
end
...
if tonumber(IdUser) == tonumber(UserId) then
Redis:del(MEZO.."AddSudosNew"..ChatId)
...
elseif Text and Text:match('(%d+)/rb:bna_MEZO') then
local UserId = Text:match('(%d+)/rb:bna_MEZO')
if tonumber(IdUser) == tonumber(UserId) then
Redis:set(MEZO.."rb:bna"..ChatId,true) 
local reply_markup = LuaTele.replyMarkup{type = 'inline',data = {{{text = ': رجوع ➡️ .', data =UserId..'/'.. 'listallAddorrem'},},}}
...
if Info_Members.members[k].bot_info == nil then
if Info_Members.members[k].status.luatele == "chatMemberStatusCreator" then
Redis:sadd(MEZO.."TheBasics:Group"..UserId[2],v.member_id.user_id) 
x = x + 1
else
...
if tonumber(IdUser) == tonumber(UserId[1]) then
Redis:del(MEZO.."Lock:tagservrbot"..UserId[2],true)   
list ={"Lock:Bot:kick","Lock:User:Name","Lock:hashtak","Lock:Cmd","Lock:Link","Lock:forward","Lock:Keyboard","Lock:geam","Lock:Photo","Lock:Animation","Lock:Video","Lock:Audio","Lock:vico","Lock:Sticker","Lock:Document","Lock:Unsupported","Lock:Markdaun","Lock:Contact","Lock:Spam"}
for i,lock in pairs(list) do 
Redis:del(MEZO..''..lock..UserId[2],"del")    
end
LuaTele.answerCallbackQuery(data.id, "𖥔 تم قفل جميع الاوامر بنجاح  ", true)
end
end
